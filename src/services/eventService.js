...
class EventService {
  static async createEvent(
    ...
    groupId = null  // Add groupId parameter
        ...
        try {
             const query = `
        INSERT INTO events 
        (user_id, name, type, datetime, description, cover_photo_url, image_urls, is_online, online_link, online_link_visible, group_id)
 ...
 static async getEventsByGroup(groupId) {
    const client = await pool.connect();
    try {
        const query = `SELECT * FROM events WHERE group_id = $1 ORDER BY datetime DESC`;
